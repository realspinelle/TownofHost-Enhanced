...
{
    public static  string Csize = "85%"; // CustomRole Settings Font-Size
    public static  string Asize = "75%";
}
...
internal class ChatCommands
{
    ...
    private static string PollMSG = "";
     ...
     // All Appended Addons Font-Size

    public static List<string> ChatHistory = [];

    public static bool Prefix(ChatController __instance)
    {
        var chatText = __instance.freeChatField.textArea.text;
        string prefix = chatText[0].ToString();
        if (prefix != "/") return true;
        string[] args = chatText.Split(' ').Skip(0).ToArray();
        string command = chatText.Split(' ')[0].Replace("/", "");
        var cmd = CommandsManager.GetCommand(command);
        if (cmd == null || !cmd.CanExecute(args, PlayerControl.LocalPlayer)) return true;
        return cmd.Execute(args, PlayerControl.LocalPlayer);
    ...
    public static void OnReceiveChat(PlayerControl player, string text, out bool canceled)
    {
        ...
        string command = chatText.Split(' ')[0].Replace("/", "");
        var cmd = CommandsManager.GetCommand(command);
        if (cmd == null || !cmd.CanExecute(args, player))
        {
            canceled = false;
            return;
        }
        canceled = cmd.Execute(args, player);
    //     canceled = false;
